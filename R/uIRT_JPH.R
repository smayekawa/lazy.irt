...
#'
#' @usage uIRT( Uc, U=NULL, items=NULL, groupvar=NULL, idvar=NULL
...
#'
#'
#' @param Uc   n x nitems+1(idvar) or +2(groupvar) 項目反応データフレーム \cr
...
#' @param U n x sum(ncat)+1(idvar) or +2(groupvar) 多値項目を one-hot に
#' 展開した項目反応データフレーム （BILOG-MG の expanded format 形式）
#' @param items Not Yet Available
#' @param   groupvar グループ名を表す U または Uc の変数名
#' @param   idvar    受験者名を表す U または Uc の変数名
#' @param   ncat     nitems x 1 項目のカテゴリ数からなるベクトル or NULL.
#' @param   type     nitems x 1 項目タイプからなるベクトル: \cr
#'          "Bn" | "G" | "PN" | "P" \cr
#'          NULL の場合は全て P であると仮定する。
#' @param itemname nitems x 1 項目名からなるベクトル or NULL.
#' @param DinP = 0 ロジスティク関数に D=1.7 を用いる。
#' @param param  nitems x max(ncat)  項目パラメタデータフレーム\cr
#'        初期値もしくは固定値を含むもので、部分的に与える場合には
#'         \code{param$name} と \code{colname(Uc)} を利用してマッチされる。
#' @param msn  nG x 3  各グループの特性値の分布の母数 (mean, std, n) の初期値
#' からなる行列
#' @param   fixeditems 項目パラメタの値を \code{param} データフレームの値に
#' 固定すべき項目名または番号からなるベクトル
#' @param  baseform 特性値の分布を固定すべきグループの番号で msn[baseform,]
#' に固定される。 \cr
#'  固定項目の項目パラメタの値は、baseform の集団における値であること。
#' @param   theta    npoints x 1  周辺最尤解のための離散の特性値 theta の値
#' @param thd NOT used.
#' @param   estmu    = 1  各グループの特性値の分布の平均値の推定を行う。
#' @param   estsigma = 1  各グループの特性値の分布の標準偏差の推定を行う。
#' @param   npoints 区間 [thmin, thmax] における theta の個数
#' @param thmin 特性値 theta の最小値
#' @param thmax 特性値 theta の最大値
#' @param   maxiter  最大繰り返し数
#' @param   eps      周辺対数尤度 lmlh の改善率の最大値を用いた収束判定基準
#' @param   epsd     各群の特性値の分布を推定する場合の \code{msn} の
#' 変化量の絶対値の最大値を用いた収束判定基準
#' @param  maxiter2  EM アルゴリズムの初期の\の m-step で用いられる
#'  \code{ordinal_reg} と \code{smn} の繰り返し数の上限
#' @param   eps2  EM アルゴリズムの初期の m-step で用いられる eps
#' @param   nstrict EM アルゴリズムの初期の定義
#' @param   maxiter22  EM アルゴリズムの後期の m-step で用いられる
#'  \code{ordinal_reg} と \code{smn} の繰り返し数の上限
#' @param minp1 項目パラメタ p1 （識別力）の最小値
#' @param maxabsparam 項目パラメタの絶対値の最大値
#'
...
#'
#' @param print = 1 結果を表示する
#' @param plot = 1 推定された特性値の分布のプロット
#' @param smallP = icrf の最小値
#' @param debug = 1 途中経過の出力
#'
#' @references

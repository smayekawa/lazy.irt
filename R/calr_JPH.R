...
#'
#' @usage calr(indata, baseform = 0, nsubj = 0
#' , npoints = 21, thmin = -3, thmax = 3
#' , maxiter = 100, eps = 0.00001, print = 1, debug = 0)
#'
#' @param indata 以下の変数を持つ項目パラメタのデータフレーム: \cr
...
#' @param baseform  基準となる集団名または 0
#' @param nsubj 各集団の受験者数または 1（重みとして使われる）
#' @param npoints 特性値 theta の離散点の数
#' @param thmin theta の最小値
#' @param thmax theta の最大値
#' @param maxiter 最大繰り返し数
#' @param eps 収束判定に用いられる rss の最大変化率
#' @param print > 0 結果を出力
#' @param debug = 1 途中結果を出力
#'
#' @details

...
plot_result <- function( result ){
  ...
  for( j in 1:(nitems) ){
    ...
    plot(theta,icrfNew[,toP]
         , xlab="", col=2, lty=1, lwd=2, main=main, cex.main=1.5
...
#            ,  pch=c(NA,21), lty=c(1,2), title="model" )
  ...
  for( j in 1:(nitems) ){
    title=paste("Item Information Function of "
                ...
                , ")  rmse_p=", rmse_pF, " (method=", method
                , ", wtype=", wtype, ")", sep="")
    main=paste("rmse_iif = ", rmse_iiF, sep="")
    matplot( theta, cbind(iifOld,iifNew)
             , pch=c(NA,21), xlab="", lty=c(1,2), lwd=2, main=main, cex.main=1.5
             , type="l", ylab="item information", ylim=c(0,maxy) )
  ...
  if( plot >= 3 ){
    ...
    for( j in 1:(nitems) ){
      title=paste("Item Category Information of "
                  ...
                  , ")  rmse_p=", rmse_pF, " (method=", method
                  , ", wtype=", wtype, ")", sep="")
      main=paste("rmse_icif = ", rmse_iicF, sep="")
      ...
      plot(theta,icifNew[,toP], main=main, sub=sub, xlab="", type="b"
           , col=2, lty=1, lwd=2, pch=ncat, cex.main=1.5, cex.sub=1.5
           , xlim=c(min(theta),max(theta)), ylim=c(0,maxy)
           , ylab="item category information")
...
plot_res <- function(){
  ...
  plot_result( res1 )
  plot_result( res2 )
  title(main=paste("method = 0", paste(rep(" ",15), collapse="")
                   ,"method = 1", paste(rep(" ",18), collapse="")
                   ,"method = 2", sep="")

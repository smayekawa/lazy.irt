...
#' @param   ncat     nitems x 1 #  of categories for each item
#' @param   type     nitems x 1  item type as   "Bn" | "G" | "PN" | "P" \cr
#'          If NULL, P will be used.
#' @param DinP = 0 to exclude 1.7 from logistic function.
#' @param param  nitems x max(ncat)  initial value data frame for item param\cr
...
#' set.seed(1701)
#' indata <- gendataIRT( 1, paramS1, npoints=500, thdist="rnorm"
#'                     , compress=1 )$U
#' indata <- as.data.frame(indata)
#' itemtype <- paramS1$type
#' res1 <- uIRT( indata, type=itemtype, maxiter=20 )
#'
#' # 2PLM with fixed nonzero c-parameter value
#' paramF <- paramS1[1,]
#' paramF$p3 <- 0.2

...
#' res1 <- LRT( Uc, type=itemtype, maxiter=200 )
#'
#' # 3PLM analysis
...
#' res1 <- LRT( Uc, type=itemtype, maxiter=20 )
#'
...
#' ncat=temp$ncat
#' rm(temp)
#' res2 <- LRT( U=U, ncat=ncat, type=itemtype, maxiter=20 )
#'
#' # should be identical
...
#' res1 <- LRT( Uc, type=itemtype, maxiter=20, param=paramF
...
#' res1 <- LRT( indata12, groupvar="group", type=itemtype, maxiter=10
...
#' res3 <- LRT( U=UN, ncat=ncat, maxiter=20 )
#'
#'
#' @export
#'

LRT <- function( Uc, U=NULL, groupvar=NULL, idvar=NULL
 ...
 getlmlh <- function( U, group, V, theta, thd ){
 ...
 fit <- function( newp ){
  ...
  # in which this function is defined. i.e., LRT.
 ...
 if( is.null(U) ){
  ...
  if( !exists(dfname) ){
   cat("\n\nerror1(LRT) **** data frame ", dfname
       , " does not exist. ****\n\n")
   return(NULL)
  }
  else if( !is.data.frame(Uc) ){
   cat("\n\nerror1(LRT) **** ", dfname, " is not a data frame. ****\n\n")
   return(NULL)
  }
  ...
  if( is.null(groupvar) ){
   ...
   if( length(locgvar) == 0 ){
    cat("\n\n error1(LRT): group variable does not exist.\n\n\n")
    Print(groupvar)
    return( NULL )
   }
  ...
  if( is.null(idvar) ){
   ...
   if( length(locidvar) == 0 ){
    cat("\n\n error1(LRT): ID variable does not exist.\n\n\n")
    Print(idvar)
    return( NULL )
   }
   #
  ...
  if( baseform <= 0  |  nG < baseform ){
   cat("\n\n error1(LRT): baseform must be in [1,",nG, "]\n\n\n")
   return( NULL )
  }
  ...
  if( is.null(ncat) ){
   cat("\n\nerror1(LRT): *** When U is given, ncat must be given. *** \n")
   return(NULL)
  }
  if( is.null(itemname) ){
 ...
 if( is.null(type) ){
  ...
  if( !( (length(grep("^B",type[ncat == 2])) == sum(ncat == 2))
   ...
   cat("\n\n error1(LRT): Item type does not match the # of categories.\n\n")
   Print(param)
   return(NULL)
  }
 }
...
#   temp=rep(1,npoints)
 ...
 if( print > 0 ){
  cat("\n\n\nLRT: parameter estimation of unidimensional IRT models\n")
 ...
 #initial
 V=irf( param, npoints=npoints, print=0 )$ICRF
Print(V)
 temp=getlmlh( U, group, V, theta, thd )
 lmlhp=temp$lmlh
 logP=temp$logP
 H=temp$H
 R=temp$R
...
#     thd[,g]=temp/sum(temp)
 ...
 return(
  list( V=V, msn=msn, theta=theta, thd=thd
...
res1 <- LRT( Uc, maxiter=1000, SQUAREM=0, print=1, npoints=npoints, estmu=1 )

res2 <- LRT( Uc, maxiter=1000, SQUAREM=0, print=1, npoints=npoints, estmu=1
, thd=rep(1/npoints, npoints) )


res1 <- LRT( Uc2, maxiter=1000, SQUAREM=0, print=1, npoints=npoints, estmu=1 )

res2 <- LRT( Uc2, maxiter=1000, SQUAREM=0, print=1, npoints=npoints, estmu=1
, thd=rep(1/npoints, npoints) )

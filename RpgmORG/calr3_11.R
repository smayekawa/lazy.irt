...
Print(res$param)
 ...
 for( llll in 1:maxiter ){
  ...
  for( j in 1:nitems1 ){
   ...
   # update parameters alng the direction d
   step=1
   ok=0
   for( lllll in 1:20 ){
 ...
 if( baseform != 0 ){
  ...
  nsb=as.vector(nsubj)
  tmean=sum(q*nsb)/nforms
  tstd=sqrt( sum((r^2 + q^2)*nsb)/nforms - tmean^2 )
  # Print(tmean,tstd)
...
# Print(indata_solo, param_solo, nfini, fini,iinf)
 ...
 return( list( param=param,uv=uv,qr=qr
  ...
  , iter=iter, rss=rss, rssimpr=rssimpr, eps=eps
  , nisolo=nisolo
...
etime(0)
res=calr( indata=paramCal1, baseform=1, maxiter=300, eps=1e-6
          , print=2, debug=1  )
etime(1)

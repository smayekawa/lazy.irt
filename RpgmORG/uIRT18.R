...
#'
#' @param Uc   n x nitems+1 or +2    compressed item response data frame
#'          in BILOG-MG's expanded format. (idvar and groupvar)
#' @param U n x sum(ncat)+1 or +2 uncompressed item response data frame
#' @param items a vector consisting of the column names of U or Uc
#' to be used as the item responses. \cr
#' Or NULL to use all columns except for groupvar and idvar. \cr
#' (Not Yet Available)
#' @param   groupvar name of the grouping variable contained as a column of Uc
#'  or NULL.
#' @param   idvar    name of the id variable contained as a column of Uc
#'  or NULL
...
uIRT <- function( Uc, U=NULL, items=NULL, groupvar=NULL, idvar=NULL
 ...
 # remove checking the existence of Uc or U: 20180419dnc
 # items: 20180419dnc,20 (Not Yet Available)
 # bugfix for groupvar: 20180511(705)
 # roxgen2: 20200102cot  (ivar bug not fixed yet)
 # aic bic: 20230217
 # roxygen2: 20231027
 #
 #
 ...
 #  groupvar name of the grouping variable contained as a column of Uc
 #  idvar    name of the id variable contained as a column of Uc
 #
 ...
 #  maxiter22  max # of iterations for ordinal_reg and smn when
 #             llll > nstrict
 ...
 #
 #  Comments:
 #   When theta distribution is estimated, mu and sigma for
 #   the baseform group is fixed to (0,1).
 #
 ...
 getlmlh <- function( U, group, param, theta, thd ){
  ...
  for( g in 1:nG ){
   ...
   ULP=U[locg,,drop=0]%*%t(logP) + matrix(1,length(locg))%*%t(log(thd[,g]))
 ...
 fit <- function( newp ){
  ...
  # in which this function is defined. i.e., uIRT.
 ...
 const <- function( newp ){
  ...
  # reshape
  newp[1:nparam1]=
   c(as.matrix(param0[nonfixeditemlist,4:(4+maxnp-1)])[locSQEM])
  return( newp )
 } # end of const
 ...
 if( is.null(U) ){
  ...
  if( is.null(groupvar) ){
   ...
   group0=group
   for( g in 1:nG ) group0[group == ugroup[g]]=g
   group=group0
   rm(group0)
  }
  nG=length(unique(group))
  ...
  if( is.null(ncat) ){
   cat("\n\nerror1(uIRT): *** When U is given, ncat must be given. *** \n")
   return(NULL)
  }
  if( is.null(itemname) ){
 ...
 for( llll in 1:maxiter ){
  ...
  if( SQUAREM  &  llll >= nSQUAREM ){
   ...
   res=iSQUAREM( pp
                 ...
                 , bof_value=NULL, always=always, reset1=reset1
                 , reset2=reset2
                 , print=print, debug=0 )
   #Print(pp,res$param)
   # reshape
 ...
 if( print > 0 ){
  ...
  cat(" aic = ", aic, ",   bic = ", bic, ",  (nparam = ", nparam," and n = "
      , n, ")\n")
 ...
 if( plot >= 1  &  nG > 1 ){
  ...
  plot(theta,thd[,nG], main=title, sub=sub
       , xlim=c(min(theta),max(theta)), ylim=c(0,maxthd)
       , type="l", ylab="prb")
  par(new=0)
 }
 ...
 return(
  list( param=param, msn=msn, theta=theta, thd=thd
        , converged=converged, lmlh=lmlh, aic=aic, bic=bic
        ,  iter_hist=iter_hist, H=H, NN=NN, EAP=EAP, poststd=poststd, id=id )
 )
...
# 2PLM analysis
itemtype <- rep("B2",nrow(paramB2))
res1 <- uIRT( Uc, param=paramC, type=itemtype, maxiter=200 )
...
itemtype <- paramS1$type
res1 <- uIRT( indata, type=itemtype, maxiter=200, SQUAREM=3, always=1
              , reset1=0, reset2=1 )
...
UN <- as.data.frame(temp$U)
UNx=2*UN
res4 <- uIRT( U=UNx, ncat=ncat, maxiter=20 )

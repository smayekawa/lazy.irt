#' Conversion to Logistic Graded Response Model
#'
#' Conversion to Logistic Graded Response Model using Weighted Least Squares \cr
#' Japanese help file: (\link[lazy.irt]{conv2G_JPH})
#'
...
#'
#' @return A list of: \cr
#' paramNew: Fitted logistic GRM item parameter data frame\cr
...
#'
#' @details
#' This function finds the set of Logistic GRM item parameters
...
#'
#' If method = 3, this function minimizes\cr
...
#'
#' @examples
#' # convert 3PLM and GPCM items to logistic GRM items
#' param <- paramA1[c(2,5,8),]
#' theta <- seq(-4,4,length=51)
#' # maxiter for method=2 is too small.
#' res1 <- conv2G( param, theta, maxiter=20, plot=1, wtype=1, method=1 )
#' res2 <- conv2G( param, theta, maxiter=2, plot=1, wtype=1, method=2 )
#'
...
conv2G <- function( param, theta=NULL, init=1, paramG=NULL
                    , method=1, wtype=1, wmean=0, wsd=1, DinP=1
                    , npoints=21, thmin=-3, thmax=3, printGN=0
                    , maxiter=500, eps=1e-6, epsg=1e-6, epsx=1e-9
                    , print=1, plot=0 ){
 # fitting Logistic Graded Response Model to Generalized Partial Credit Model
 # Shin-ichi Mayekawa
 ...
 # use iif: 20221206
 # converted from fotGm2P_lsd: 20221206
 # roxygen2: 20221207
 # return variables renamed: 20221208
 # bugfix for init=1: 20221210
 # print less: Normal -> Logistic: 20230202
 # epsg, epsx: 20230202
 # return type B2 items: 20230213
 # renamed as conv2G: 20230220,21
 ...
 if( is.data.frame(paramP) ){
  # param
  isdf=1
  types=c("B3","Bn","Bn3","P","Gn")
  paramP=paramP[paramP[,"type"] %in% types,]
  if( nrow(paramP) == 0 ){
    cat("\n\nerror1:(conv2G) Input Item Parameter ERROR.\n")
 ...
 # Here, paramP contains only GPCM items ("B3","Bn","Bn3","P").
 ...
 # Print(ic_infoP,fromP,toP, paramP, icrf,dicrf)
...
temp=conv2G( paramA1[c(2,8,9),], plot=1, print=1, wtype=1, method=1, init=1 )
...
res20=fitG2P_ls( paramA1[8,], plot=1, print=4, wtype=1, method=0, init=0 )
res2=conv2G( paramA1[8,], plot=1, print=4, wtype=1, method=1, init=0 )

res20=fitG2P_ls( paramA1[8,], plot=1, print=4, wtype=1, method=2, init=0 )
res2=conv2G( paramA1[8,], plot=1, print=4, wtype=1, method=2, init=0 )



itemparam=paramA1[c(3,2,4),]

res0=fitG2P_ls( itemparam, plot=4, print=1, wtype=1, method=0, init=0 )
rmse_p0=res0$rmse_p; rmse_ii0=res0$rmse_ii; rmse_iic0=res0$rmse_iic

res1=fitG2P_ls( itemparam, plot=4, print=1, wtype=1, method=1, init=0 )
rmse_p1=res1$rmse_p; rmse_ii1=res1$rmse_ii; rmse_iic1=res1$rmse_iic

res2=fitG2P_ls( itemparam, plot=4, print=1, wtype=1, method=2, init=0 )
...
res=fitG2P_ls( parambad, theta, plot=1, print=1, wtype=1, method=2 )
...
# res=fitG2P_ls( param, theta, plot=1, print=2, maxiter=50, wtype=1, method=0 )
res=fitG2P_ls( param, theta, plot=1, print=1, wtype=1, method=1 )

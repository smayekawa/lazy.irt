...
flatten_SEM <- function( out_obscore=NULL, sigma=1, t_by=0.5
 ...
 # new brk, use TRF instead of t: 20170308
 #
 ...
 # sem of Y=g(X) at t
 gdash=get_gdash( t )
 stdy_u=gdash*stdx_t    # This should be exactly equal to sigma.

 # discretize t to get u
 ump=seq(floor(min(u)),ceiling(max(u)))
 ubrk=midp2brk(ump)
 brk=interpol( u,t, ubrk )[,2]
 Print(ump,ubrk,brk)
 brk_x2u=cbind(ubrk,brk)
 # Print(u,ubrk,brk_x2u)

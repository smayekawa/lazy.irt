...
#' @param min_theta Minimum value of theta for method="MLE" or "MAP".
#' @param max_theta Maximum value of theta for method="MLE" or "MAP".
...
est_theta <- function( Uc=NULL, U=NULL, param=NULL, method="EAP"
 ...
 # thetastd for EAP and ML: 20230202
 # return thetahat and thetastd as vectors: 20230202
 # bug in error report: 20230213
 # min_theta and max_theta: 20230220
 #
 ...
 if( !is.null(Uc) ){
  ...
  if( nitem != ncol(Uc) ){
   cat("\nerror1:(est_theta) # of items in param = ", nitem
       , " but # of columns in input data = ", ncol(Uc), "\n" )
   return()
  }
  # uncompress data
...
# Print(rowmin,rowmax,rowmin1,rowmax1)
  ...
  thetastd=c( sqrt( H%*%(theta^2)-thetahat^2 ) )
  ...
  for( i in 1:N ){
   ...
   thetahat[i]=optimize( mllh_theta, c(min_theta,max_theta), u=ui )$minimum
  }

  thetastd=c( sqrt( 1/iif( param, theta=thetahat, print=0 )$info ) )
 ...
 res=named_list( thetahat, thetastd, theta, thd
...
plot(theta,res$thetahat)
plot(theta,res$thetastd)

res2 <- est_theta( U=U, param=paramS3, print=1, method="ML" )
plot(theta,res2$thetahat)
plot(theta,res2$thetastd)

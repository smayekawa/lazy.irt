...
#'     scaled so that v[0,j]=0, j=1,2,...,nitems, and v[1,1]=1.
#'
#' @details
...
info_func <- function( param, weight=NULL
 ...
 # locally best category weight: scaled so that v[0,j]=0 and v[1,1]=1.
 dd=dicrf/icrf
 ...
 v_LO=matrix(unlist(v_LO),npoints)
 colnames(v_LO)=colnames(dicrf)
 rownames(v_LO)=thname
 ...
 if( print > 0 ){
  ...
  if( print >= 3 ){
   ...
   Print(w_LO, fmt="6.3")
   cat(" Locally Best Item Categorie Weights: scaled so that"
       , " v[0,j]=0 and v[1,1]=1. \n")
   Print(v_LO, fmt="6.3")
  }
 }
 ...
 res=named_list( theta, info, fromP, toP, TRF, TRF_LO
               , info_LO, info_LOW, info_item_LO, info_item_LO,  w_LO, v_LO )
...
resInfo=info_func( paramS2, plot=3, print=3, npoints=51 )

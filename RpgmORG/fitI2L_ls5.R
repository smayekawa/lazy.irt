...
#' = 0 to use no weight.
#' @param useoptim = 0 to use optimize instead.
...
fitI2L_ls <- function( V, print=0, plot=0, title=NULL, wtype=0, useoptim=1
 ...
 # constant w: 20180116
 # optim for theta -> optimize: 20180116
 #
 ...
 get_ab <- function( param2, theta, Pj, w
  ...
  rmsej=sqrt( sum( w*(Pj-Pjhat)^2 )/npoint )
  rmsejp=rmsej
  ...
  for( llll in  1:maxiter ){
   ...
   # Jacobian:  d vec(Pj) / d paramj    ncatj*npoint x ncatj
   ...
   for( lllll in 1:20 ){
    ...
    Pjhat=c( irf( paramjdf, theta, print=0, zero=0 )$ICRF )
    rmse1=sqrt( sum( w*(Pj-Pjhat)^2 )/npoint )
 ...
 rssthq <- function( thq, tVq, param ){
  rss=tVq-icrfB( param, thq )
  rss=sum( rss*rss )
  return( rss )
 } # end of rssth
 ...
 # item parameter df
 paramdf=data.frame( name=paste("Q",1:nitem,sep=""), type="B", ncat=2
                     , stringsAsFactors=0)
 paramdf=cbind(paramdf,param)
 ...
 for( llll in 1:maxiter ){
  ...
  rss=rssab( param, tV, w )
  rmse=sqrt(rss/nitem/npoint)
  if( print >= 3 ) Print("after ab", rmsep, rmse)
  ...
  if( useoptim ){
   ...
   paramdf[,c("p1","p2","p3")]=param
   # theta=get_theta( theta, paramdf, tV, w, maxiter=50, print=2 )

   for( q in 1:npoint ){
    thq=theta[q]
    thq=optimize( rssthq, c(-3,3), tVq=tV[q,], param=param )$minimum
    theta[q]=thq
   }
  }
  ...
  if( print >= 2 ){
   Print( llll, rmsep, rmse, rmseimpr )
  }
...
etime(0)
res=fitI2L_ls( V2, print=1, plot=1, wtype=0 )
etime(1)

...
#'
#' @return
#' A data frame of nitem x max(ncat) consisting of name, type, ncat and
#'  the modes
#' corresponding to the 0-th, 1st, 2nd, ..., and ncat-th category.
#' \cr
#' Note that the first (0) and the last (ncat) columns are filled with NAs.
#'
#' @examples
...
find_mode <- function( param, print=0 ){
 ...
 # Shin-ichi Mayekawa
 # 20161122dnc
 # Gn: 20180214FacultyMeeting
 # return data frame: 20180214
 #
...
#'
#' @return
#' A data frame of nitem x max(ncat)-1 consisting of name, type, ncat and
...
find_intersection <- function( param, print=0 ){
 ...
 for( j in 1:nitem ){
  ...
  if( typej %in% c("G", "Gn", "B3") ){
   # G or B3
   modej=as.matrix(find_mode( paramj )[,-(1:3)])
 ...
 intersectdf=param[,1:3]
 intersectdf=cbind(intersectdf,intersect)
...
intersect=find_intersection( paramA1 )
Print(intersect, fmt="8.3")


mode=find_mode( paramA1 )
Print(mode, missing=".")

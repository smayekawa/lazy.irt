...
#'
#' Conversion to Generalized Partial Credit  Model 
#' using Weighted Least Squares \cr
#' Japanese help file: (\link[lazy.irt]{conv2P_JPH})
#'
...
conv2P <- function( param, theta=NULL, init=1, paramP=NULL
 ...
 # renamed as conv2P: 20230220,21
 # method=0 -> method=1: 20230220
 # paramG -> param: 20230221
 ...
 if( is.data.frame(paramG) ){
  # param
  isdf=1
  types=c("Bn","B3","Bn3","G","Gn")
  paramG=paramG[paramG[,"type"] %in% types,]
  if( nrow(paramG) == 0 ){
   cat("\n\nerror1:(conv2P) Input Item Parameter ERROR.\n")
 ...
 paramG0[paramG0$type %in% types,]=paramP
 paramP=paramG0

 return( named_list( paramNew=paramP, paramOld=param, method
...
temp=conv2P( paramA1[c(2,8,9),], plot=1, print=9, wtype=1, method=1, init=1 )
...
res20=fitP2G_ls( paramA1[9,], plot=1, print=9, wtype=1, method=0, init=1 )
res2=conv2P( paramA1[9,], plot=1, print=9, wtype=1, method=1, init=1 )

res20=fitP2G_ls( paramA1[9,], plot=1, print=9, wtype=1, method=2, init=1 )
res2=conv2P( paramA1[9,], plot=1, print=9, wtype=1, method=2, init=1 )

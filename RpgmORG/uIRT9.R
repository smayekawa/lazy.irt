...
maxiter=250;
# maxiter=2
#=rbind(indata,indata,indata,indata,indata,indata,indata,indata)
#indata1=rbind(indata1,indata1,indata1,indata1,indata1,indata1)
#indata1=rbind(indata1,indata1,indata1)
#indatax=indata[,1:12]
...
comments('
etime(0)
res1=uIRT( indatax, "group", npoints=21, thmin=-4, thmax=4, type=typex
           , print=2, estmu=1, estsigma=1, baseform=2, fixeditems=NULL
           , maxiter=maxiter, eps=1e-7, epsd=1e-6, minalpha=-6
           , plot=0 )
etime()
hist1=res1$iter_hist
plot(hist1,type="b")
plot(hist1[(50<=hist1[,1])&(hist1[,1]<=90),],type="b")
plot(hist1[(2<=hist1[,1])&(hist1[,1]<=90),],type="b")
plot(hist1[,1],hist1[,5],type="b")
plot(hist1[3:nrow(hist1),1],hist1[3:nrow(hist1),3],type="b")
')



etime(0)
res0=uIRT( indatax, "group", npoints=21, thmin=-4, thmax=4, type=typex
           , print=2, estmu=1, estsigma=1, baseform=2
           , fixeditems=(colnames(indatax)[-1])[c(1,2,3,15,16,17,25,26,27)]
           , maxiter=maxiter, eps=1e-7, epsd=1e-6, plot=0, SQUAREM=3
           , param=pp, debug=0 )
etime()
comments('
hist1=res0$iter_hist
plot(hist1[2:nrow(hist1),],type="b")
plot(hist1[10:nrow(hist1),],type="b")
plot(hist1[20:nrow(hist1),],type="b")
plot(hist1[50:nrow(hist1),],type="b")
plot(hist1[70:nrow(hist1),],type="b")
')
stop()

etime(0)
res01=uIRT( indatax, "group", npoints=21, thmin=-4, thmax=4, type=typex
            , param=res0$param[c(1,7,13,27,48),], fixeditems=NULL
            , print=2, estmu=1, estsigma=1, baseform=2
            , maxiter=maxiter, eps=1e-7, epsd=1e-4, plot=0 )
etime()
...
etime(0)
res01=uIRT( indatax, "group", npoints=21, thmin=-4, thmax=4, type=typex
           , param=res0$param[c(1,7,13,27,48),]
           , fixeditems=res0$param$name[c(1,7,13,27,48)]
           , print=2, estmu=0, estsigma=0, baseform=2
           , maxiter=maxiter, eps=1e-7, epsd=1e-4, plot=0 )
etime()
...
#irf(res0$param[c(36,48),], theta, plot=1, print=0)
#irf(res0$param[c(35,47),], theta, plot=1, print=0)
#irf(res0$param[c(34,46),], theta, plot=1, print=0)
#irf(res0$param[c(33,45),], theta, plot=1, print=0)
#irf(res0$param[c(25,37),], theta, plot=1, print=0)

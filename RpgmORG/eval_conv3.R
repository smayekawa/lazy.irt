...
doit <- function( mode, outroot="RpgmORG/stat/", ncat=3
 ...
 fitG <- function( method, print=1 ){

  # convert
  res2=fitG2P_ls( param1, theta, wtype=1, method=method, print=print )
  param2=res2$paramG
 ...
 fitGn <- function( method, print=1 ){

  # convert
  res2=fitGn2P_ls( param1, theta, wtype=1, method=method, print=print )
  param2=res2$paramG
 ...
 fitP <- function( method, print=1 ){

  # convert
  res2=fitP2G_ls( param1, theta, wtype=1, method=method, print=print )
  param2=res2$paramP
 ...
 sname0=c( "ncat", paste("p1",1:ncat,sep=""), paste("th1",0:(ncat-1),sep="")
           , "a_iinfo1" )
 sname=c( "method", paste("p2",1:ncat,sep="") )
 sname=c( sname, "rmse_p","rmse_ii","rmse_iic","rmse_t","rmse_tt" )
 sname=c( sname, paste("th2",0:(ncat-1) ,sep=""), "a_iinfo2" )
 sname=c(sname0,sname)
...
res1=doit( mode="G2P", outroot=outroot, ncat=ncat,  nrep=nrep, seed=1702 )
res2=doit( mode="Gn2P", outroot=outroot, ncat=ncat,  nrep=nrep, seed=1702 )
res3=doit( mode="P2G", outroot=outroot, ncat=ncat,  nrep=nrep, seed=1702 )
res4=doit( mode="P2Gn", outroot=outroot, ncat=ncat,  nrep=nrep, seed=1702 )

statdf=rbind(res1,res2,res3,res4)

outfile=paste(outroot, "stat_all.csv", sep="" )
write.table( statdf, outfile, row.names=FALSE, sep=", " )

...
abv=dnorm(theta)%*%resInfo$v_LO/sum(dnorm(theta))
cn=colnames(abv)
abv=c(abv)
round(abv, digits=3)
Print(cn, abv,diff(abv))

max(abs(resInfo$info_LOW-resInfo$info_LO))
...
matplot(theta,tt, type="l", main="Best Item Score", lwd=2)

matplot( theta,tt[,5:10], type="l"
         , col=c("black", "red", "green", "blue"), lty=c(1:3), lwd=2
         , xlim=c(min(theta),max(theta)), ylim=c(0,max(tt))
         , main="Item Response Function with Best Weight", ylab="score")
 legend( range(theta)[1], max(tt)-.1, iname[5:10]

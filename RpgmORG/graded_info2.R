...
graded_info <- function( out_obscore, ncat=5, method=1, miny=0, brk=NULL
                         , print=1, plot=0 ){
 ...
 if( print ){
  ...
  cat("\n ", brk,"\n")
  Print(scorex,brk,brk_theta, scorey)
 }
...
Print(ncat,npoint,scorey,thname)
 ...
 if( plot ){
  ...
  temp=cbind(stdx_t/(maxscorex-minscorex),stdy_t/maxscorey)
  title=paste("Comparison of relative SEMs: SEM/maxscore:  ncat = ", ncat
              , " (method = ", method, ")", sep="")
  matplot( theta,temp, type = "l", ylab="std of score"
           , lty=1, col=2:5, cex=1, lwd=1
           , main=title
 ...
 res=list( theta=theta, info=info, infoX=infox, infoY=infoy
           , Py_t=Py_t, Pt_y=Pt_y, meant_y=meant_y, stdt_y=stdt_y )

...
graded_info <- function( out_obscore, ncat=5, method=1
 ...
 # break points of X and T to get Y and S
 if( !is.null(brk) ){
  ncat=length(brk)-1
  method=0
  xmidp=brk2midp( brk )
 ...
 # break point of theta correspoinding to brk
...
#  cat("\n ", brk,"\n")
 ...
 # TRF test response function of Y etc
 TRFy_t=c( scorey%*%Py_t )
 stdy_t=c( sqrt( (scorey^2)%*%Py_t - TRFy_t^2 ) )

 #stdy_t=c( sqrt( apply(cbind(0,(scorey^2)%*%Py_t-TRFy_t^2),1,max) ) )
 ...
 if( plot ){
  ...
  title=paste("Score Distribution of X:  ncat = ",ncat
              , " (method = ", method, ")", sep="")
  title2=paste("score range:  X(",minscorex,",",maxscorex,"),  Y("
               ,minscorey,",",maxscorey,")", sep="")
  plot_freqdist( scorex, Px, title=title, sub=title2)

  title=paste("Score Distribution of Y:  ncat = ",ncat
              , " (method = ", method, ")", sep="")
  title2=paste("score range:  X(",minscorex,",",maxscorex,"),  Y("
               ,minscorey,",",maxscorey,")", sep="")
  plot_freqdist( scorey, Py, title=title, sub=title2 )
  ...
  legend( 2,max(temp)-0.2, legend=c("E(Y|theta)","SEM(theta)")
          , cex=1, lwd=1, col=1:2, merge=TRUE )

  title=paste("Plot of SEM of Y:  ncat = ",ncat
              , " (method = ", method, ")", sep="")
  title2=paste("score range:  X(",minscorex,",",maxscorex,"),  Y("
               ,minscorey,",",maxscorey,")", sep="")
 ...
 res=named_list( theta, tcc, stdx_t, info, infoX, infoY, TRFy_t, stdy_t
           , dTRFy_t, Py_t, Pt_y, meant_y, stdt_y, scorey, brk )
...
out_obscore=obscore( pp, weight=NULL, npoints=161, print=0, plot=3 )

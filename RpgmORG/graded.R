graded <- function( out_obscore, ncat, method=1, brk=NULL ){
 # calculate the distribution of the graded score
 # Shin-ichi Mayekawa
 # 20150313,14
 #
 #
 
 # How cut and breaks works: example of ncat=3
 #
 #    y=1     brk[1]  <  x  <= brk[2]
 #    y=2     brk[2]  <  x  <= brk[3]
 #    y=3     brk[3]  <  x  <= brk[4]
 ...
 if( !is.null(brk) ) ncat=length(brk)+1
  ...
  if( method == 1 ){
   ...
   brk=wqtl( scorex, Px, seq(0,1,len=ncat+1) )
   brk[1]=minscorex-0.5; brk[ncat+1]=maxscorex+0.5
  }
 }
 ncat=length(brk)+1
 
 Print( ncat0,ncat,method, brk )
 ...
 Print( theta,TRFy, dTRFy, stdy_t, infoy, info, fmt=".2" )
...
wqtl <- function( x, weight=1, probs=c(.25,.5,.75) ){
 # weighted quantile
 # Shin-ichi Mayekawa
 # 20150312
 #
 #
 #  Args:
 #   (x, weight)   freq dist or x:  Pr( X=x[i] ) = weight[i]
 #
 #  Values:
 #    quantiles    vector of length(probs) 
 #
 ...
 # correction for integer
 x=x+.5
 qt=approx( cdf, x, xout=probs )$y
 names(qt)=probs
...
max=100
ncat=3
cc=cut(c(min,max),b=ncat)
levels=attributes(cc)$levels
print(cc)
print(levels)
breaks=seq(min,max,len=ncat+1)
print(breaks)


n=1000
score=0:100
prob=dnorm(score,50,20)
prob=prob/sum(prob)
x=sample(score,n,replace=1, prob=prob)
f=unclass(table(x))
attributes(f)=NULL
cdf=cumsum(f)/sum(f)
xu=sort(unique(x))
...
for( ncat in 2:9 )
 Print(ncat, seq(0,1,len=ncat+1) )

')


comments('
out_obscore=obscore( param, weight=NULL, npoints=91
                   , thmin=-4, thmax=4, thdist=1
                   , compress=0, plot=0, print=2, debug=0 )
...
prob=dnorm(score,50,20)
prob=prob/sum(prob)
graded( out_obscore, ncat=5, method=1, brk=c(-1:26 ) )

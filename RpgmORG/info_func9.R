...
#' @param thdist = "NORMAL" or "UNIFORM" \cr
...
info_func <- function( param, theta=NULL, weight=NULL
                       ...
                       , thdist="NORMAL", thmean=0, thstd=1
 ...
 if( is.null(weight) ){
  ...
  weight=data.frame(iname, type, ncat,w,v, row.names=NULL )
  if( length(unique(iname)) == length(iname) ) rownames(weight)=iname
 ...
 if( print > 0 ){
  ...
  print( weight )
  cat("\n")
  cat("\n Theta distribution =", thdist)
  if( thdist == "NORMAL" )
   cat(" with mean =", thmean, " and std =", thstd)
  cat("\n")
 }


 # calculate icrf and trf=cond. mean of X given theta
 temp=irf( param, theta, weight, print=0, debug=0, plot=0, smallP=smallP )
 icrf=temp$ICRF
 TRF=temp$TRF
 fromP=temp$fromP
 toP=temp$toP
 ...
 # information function with the locally best item weights
 info_item_LOW=(dirf^2)/varuv_t
 info_LOW=rowSums(info_item_LOW)
 names(info_LOW)=thname

 # average info
 a_stdx_t=sqrt( sum(thd*(stdx_t^2)) )
 a_info=sum(thd*info)
 a_info_LO=sum(thd*info_LO)
 a_info_LOW=sum(thd*info_LOW)
 a_info_item_LOW=c(thd%*%info_item_LOW)
 a_info_item_LO=c(thd%*%info_item_LO)
 names(a_info_item_LOW)=colnames(info_item_LOW)
 names(a_info_item_LO)=colnames(info_item_LO)
 ...
 a_w_LO=c(thd%*%w_LO)
 a_v_LO=c(thd%*%v_LO)
 names(a_w_LO)=colnames(w_LO)
 names(a_v_LO)=colnames(v_LO)
 ...
 if( print ){
  cat("\nAverage Informations and Weights with respect to the"
     , "theta distribution\n")
#  cat("\n Theta distribution =", thdist)
#  if( thdist == "NORMAL" )
#   cat(" with mean =", thmean, " and std =", thstd)
#  cat("\n")
  cat("\n Square root of the average of SEM(theta)^2 =", a_stdx_t)
  Print( a_info, a_info_LOW, a_info_LOW )
  Print( a_info_item_LOW, a_info_item_LO, printrownames=1 )
  Print( a_w_LO, a_v_LO, printrownames=1 )

  if( print >= 2 ){
   cat( " info is the information function associated with given category and"
        , "item weights,\n"
        , "info_LO uses best category and item weights,\n"
 ...
 if( plot > 0 ){
  ...
  # SEM
  matplot( theta, stdx_t, type="l", col=1, lwd=2, lty=c(1,3)
           , main="Standard Error of Measurement (SEM) at theta" )


  # plot test info
  matplot( theta,cbind(info, info_LO, info_LOW), type="l", lwd=2
  ...
  if( plot >= 3 ){
   ...
   for( j in 1:(nitems) ){
    ...
    matplot(theta,v_LO[,fromP[j]:toP[j]], main=title, sub=sub, lwd=2
         , xlim=c(min(theta),max(theta)), ylim=c(0,maxi)
         , type="l", ylab="weight")
    if( legend > 0 ){
     legend( range(theta)[2]-1, range(v_LO)[2], 0:(ncat[j]-1)
             , cex=0.8, col=c("black", "red", "green", "blue")
             , pch=NA, lty=c(1:3), title="legend" )
    }
   }
  ...
  if( plot >= 4 ){
   maxi=max(A)
   mini=min(A)
   matplot( theta,A, type="l"
            , col=c("black", "red", "green", "blue"), lty=c(1:3), lwd=2
            , xlim=c(min(theta),max(theta)), ylim=c(mini,maxi)
            , main="Basic Functions for All Items", ylab="weight")
   if( legend > 0 ){
    legend( range(theta)[2]-1, range(A)[2], iname
            , cex=0.8, col=c("black", "red", "green", "blue")
            , pch=NA, lty=c(1:3), title="legend" )
   }
   for( j in 1:(nitems) ){
    title=paste("Basic Functions of"
                , iname[j]," ( type = ",type[j],", ncat=",ncat[j], " )")
    pp=param[j,4:(ncat[j]+3)]
    pp[is.na(pp)]=0
    sub=paste( "param = ", paste(format(pp,digits=3)
    ...
    matplot(theta,A[,fromP[j]:toP[j]], main=title, sub=sub
            , col=c("black", "red", "green", "blue"), lty=c(1:3), lwd=2
            , xlim=c(min(theta),max(theta)), ylim=c(mini,maxi)
            , type="l", ylab="weight")
    if( legend > 0 ){
     legend( range(theta)[2]-1, range(A)[2], 0:(ncat[j]-1)
             , cex=0.8, col=c("black", "red", "green", "blue")
             , pch=NA, lty=c(1:3), title="legend" )
    }
   }
 ...
 res=named_list( theta, thd, info, fromP, toP, TRF, TRF_LO, icrf, dicrf
               , info_LO, info_LOW, info_item_LO, info_item_LOW, stdx_t
               , w_LO, v_LO, A=A, a_stdx_t
               , a_info, a_info_LOW, a_info_LO
               , a_info_item_LO, a_info_item_LOW, a_info_item_LO
               , a_w_LO, a_v_LO )

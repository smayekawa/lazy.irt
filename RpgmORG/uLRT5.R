...
uLRT <- function( Uc, U=NULL, groupvar=NULL, idvar=NULL
 ...
 mbinreg <- function( r, n, A=NULL, bb=NULL, init=1
                      , maxiter=500, eps=1e-9, print=0 ){
  ...
  # monotone initial by isotonic reg
  pi00=isoreg(pi0)$yf
  # pi000=mlsreg( pi0, 1:nobs, 1/( n*pi0*(1-pi0) ) )



  # A matrix
  if( is.null(A) ){
   A=matrix(0,nobs,nobs)
   for( i in 1:nobs ){
    A[i,1:i]=1
   }
  }
  ...
  if( is.null(bb) ){
   ...
   expbb=solve(A)%*%logit1( pi00, D=1 )
   expbb1=expbb[1]
   expbb[expbb<1e-15]=1e-15
  ...
  res=nlm( get_ll, bb, iterlim=maxiter, gradtol=eps )
  llll=res$iterations
  bb=res$estimate
  bb1=bb[1]
  pi=logistic1( A%*%c(bb1,exp(bb[-1])), D=1 )
  ll=-res$minimum
  code=res$code

  grad=JacobianMat( bb, get_ll )

  if( print > 0 ){
   cat("\nIteration converged with ", llll, " iterations. \n")
   Print(r,n, pi0, pi)
   Print(bb,grad)
   Print(ll, ll00, code)
  }

  return( named_list(pi,bb,ll,ll00, code) )
   ...
   if( !( j %in% fixeditemlist) ){
    ...
    if( monotone ){
     ...
     if( any( diff(Rj[,2]/Nj) < 0 ) ){
      temp=mbinreg( Rj[,2], Nj, A, maxiter=20 )
      if( 0  &  temp$code != 1 ){
       Print(j, temp$code)
      }
      pij=temp$pi

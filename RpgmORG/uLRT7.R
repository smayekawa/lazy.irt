...
#' @param monotone = 1 to enforce monotonicity
...
uLRT <- function( Uc, U=NULL, groupvar=NULL, idvar=NULL
 ...
 # rho in SQUAREM: 20170825
 # roxgen comments: 20180112
 #
 #
 #
 # Args:
 ...
 fit <- function( newp ){
  ...
  # decompose
  if( estrho == 1 ){
   v=newp[1:(nclass*nitems*2)]
   rho=newp[-(1:(nclass*nitems*2))]
  }
 ...
 const <- function( newp ){
  ...
  # 20170620
  # rho: 20170825,28
  # check if V sums to 1 for each item: 20170829dnc
  ...
  if( estrho == 1 ){
   v=newp[1:(nclass*nitems*2)]
   rho=matrix(newp[-(1:(nclass*nitems*2))],nclass)
  }

  # reshape parameters and enforce constraints
  V=matrix(v,nclass)
  V[V<vmin]=vmin
  V[V>vmax]=vmax
  V[,fromP]=1-V[,toP]
...
#  ')
 ...
 for( llll in 1:maxiter ){
  ...
  if( SQUAREM  &  llll >= nSQUAREM ){
   ...
   #Print(pp,res$param)
   # reshape
   v=res$param[1:(nclass*nitems*2)]
   if( estrho == 1 ){
    rho=matrix(res$param[-(1:(nclass*nitems*2))],nclass)
   }
   V=matrix(v,nclass)
 ...
 if( plot ){
  alphaf=formatC(alpha, wid=6, digits=3, format="f")
  sub=paste("prior Dirichle alpha = ", paste(alphaf, collapse=" "), sep="")

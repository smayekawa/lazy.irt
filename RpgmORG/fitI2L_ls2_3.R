...
#Print(param,paramj)
   ...
   vj=tV[,j]
   w=1
   paramj=get_ab( paramj, theta, vj, w, maxiter=29, print=0 )
   param[j,1:2]=paramj

  } # end of j

  rss=rssab( param, tV, wtype=wtype )
  # Print( llll, param, rss )

  # theta points
  # res=nlm( rssth, theta, iterlim=5, gradtol=eps, tV=tV, wtype=wtype )
  # theta=res$estimate
  if( is.null(method) ){
   res=optim( theta, rssth, control=control, tV=tV, wtype=wtype )
  }
  else{
   res=optim( theta, rssth, control=control, tV=tV, wtype=wtype
             , method=method )
...
comments(
 ...
 V0=read.csv("d:/RPGM/LRT/data2/V_32.csv", header=1, stringsAsFactors=0)

...
#'
#' @details
#' This function checks if the input \code{param} is a valid parameter
#' data frame with \code{name, type, ncat, p1, p2, p3, ...,}  variables.\cr
#' When \code{type = (B | B3 | Bn | Bn3 | G | Gn | P |  PN | N)} is given,
...
checkparam <- function( param, type=NULL, modulename="checkparam"
 ...
 if( is.data.frame(param) ){
  ...
  if( !is.null(type) ){
   ...
   else if( type == "B2" ) loc=grep("^B$", param$type)
   else if( type == "Bn" ) loc=grep("^Bn[[:digit:]]*$", param$type)
   else if( type == "Bn2" ) loc=grep("^Bn$", param$type)

 ...
 if( plot ){
  ...
  if( plot >= 3 ){
   ...
   for( j in 1:(nitems) ){
    ...
    for( k in 1:(ncat[j]) ){
     plot(theta,ic_infoP[,fromP[j]+k-1], xlab="", col=1, lty=1, lwd=2
          , xlim=c(min(theta),max(theta)), ylim=c(0,maxy), type="l", ylab="")
     par(new=1)
    }
    for( k in 1:(ncat[j]-1) ){
     plot(theta,ic_infoG[,fromP[j]+k-1], xlab="", col=2, lty=1, lwd=2
...
res=fitGn2P_ls( paramA1[8,], theta, plot=1, print=1, wtype=1, method=0, init=0 )
...
res0=fitGn2P_ls( paramA1[c(5,8),], theta, plot=1, print=1, wtype=1, method=0, init=0 )
rmse_p0=res0$rmse_p; rmse_ii0=res0$rmse_ii; rmse_iic0=res0$rmse_iic

res1=fitGn2P_ls( paramA1[c(5,8),], theta, plot=1, print=1, wtype=1, method=1, init=0 )
rmse_p1=res1$rmse_p; rmse_ii1=res1$rmse_ii; rmse_iic1=res1$rmse_iic

res2=fitGn2P_ls( paramA1[c(5,8),], theta, plot=1, print=1, wtype=1, method=2, init=0 )

...
#' @param init  = 1 to use fitP2G, else use equally spaced b-parameters \cr
#' Setting this equal to 0 may result in better solution, 
#' especially in fitP2G_ls.
...
fitP2G_ls <- function( paramG, theta=NULL, init=1
 ...
 if( init == 1 ){
  ...
  if( length(locG) > 0 ){
   # GG$type="P"
   for( j in 1:length(locG) ){
    GG[j,4+(1:(GG[j,"ncat"]-1))]=sort(GG[j,4+(1:(GG[j,"ncat"]-1))])
   }
 ...
 if( print ){
  ...
  for( j in 1: nitems ){
   ...
   for( llll in  1:maxiter ){
    ...
    # Jac=JacobianMat( paramj, icrf, ..eps.. = 1e-06 )
    paramjdf[1,3+(1:ncatj)]=paramj
    if( method == 2 ) Jac=w2*JacobianMat( paramj, info_ic )
 ...
 if( plot ){
  ...
  for( j in 1:(nitems) ){
   ...
   matplot( theta, cbind(ii,iih), main=title, sub=sub,  pch=c(NA,21), xlab=""
            , type="l", ylab="information", ylim=c(0,maxy)
            , lty=c(1,2), lwd=2 )
...
res <- fitP2G_ls( paramA1[6,], plot=1, print=2, method=1 )



param1=data.frame( name="Q1", type="G", ncat=4, p1=1, p2=-1.217505
, p3=1.807205, p4=2.22728, stringsAsFactors=0 )
res <- fitP2G_ls( param1, plot=1, print=1, method=1 )
...
# res=fitP2G_ls( param, theta, plot=1, print=1, maxiter=50, wtype=1, method=0 )

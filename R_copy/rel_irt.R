...
rel_irt <- function( param, weight=NULL, maxscore=100, npoints=51
 ...
 if( plot ){
  ...
  upper=trf+stdx_t
  lower=trf-stdx_t
  title="Test Response Function with SE"
  ...
  leg=c(leg, paste("Y:  average SEM ="
                   , formatC(aSEM_ys,digits=2, width=6, format="f")
        , ",  rel = ", formatC(rel_ys,digits=2, width=4, format="f")
        , sep="") )
  title1="Plot of SEs as the function of Theta"
  matplot( theta, cbind(SE_x,SE_y), type="l", main=title1, ylab="SE", col=1
           , ylim=c(0,max(c(SE_x,SE_y))) )
  legend("top", legend=leg, col = 1, lty = 1:2)
  title2=paste( "Plot of SEs as the function of Score: "
                , " theta distribution =", thdisttype[thdist+1], sep="" )
  matplot( score, cbind(SE_xs,SE_ys), type="l", main=title2, ylab="SE", col=1
           , ylim=c(0,max(c(SE_x,SE_y))) )
  legend("top", legend=leg, col = 1, lty = 1:2)
